        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              className="w-full rounded-xl bg-blue-500 hover:bg-blue-600 py-3 text-sm font-semibold text-white transition-all shadow-sm"
            >
          ...
          <div>
            {result ? (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
                  ...
                  {result.eligiblePrograms.length > 0 ? (
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <div className="mt-2 flex flex-wrap gap-1">
                                {p.highlights.map((h) => (
                                  <span key={h} className="rounded-md bg-gray-100 px-2 py-0.5 text-xs text-gray-600">
                                    {h}
                                  </span>
                                ))}
                              </div>
                    ...
                    <div className="mt-4 flex items-start gap-2 rounded-xl border border-orange-200 bg-orange-50 p-4">
                ...
                {form.hasOverseasProperty && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl border border-red-200 bg-red-50 p-6">
              ...
              <div className="flex h-full items-center justify-center rounded-2xl border border-gray-100 bg-white p-12">
...
function InputField({
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 transition-colors focus-within:border-blue-400">
...
function CostRow({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className="flex justify-between text-sm">
      <span className={highlight ? "text-orange-500" : "text-gray-500"}>{label}</span>
      <span className={highlight ? "text-orange-500" : "text-gray-900"}>${value.toLocaleString()}</span>
    </div>
  );
}
...
function DTICard({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div className={`rounded-xl border p-4 ${isOver ? "border-red-200 bg-red-50" : "border-gray-100 bg-white"}`}>
      <div className="text-xs text-gray-500">{label}</div>
      <div className={`mt-1 text-2xl font-bold ${isOver ? "text-red-500" : "text-blue-600"}`}>
        {value}%
      </div>
